...
class MGDataset(torch_geometric.data.Dataset):
    ...
    def __init__(self, root, directed=False, transform=None, edge_target_csv='targets_e_class.csv', use_cache=True, rdkit_config: Config = None):
        ...
        try:
            self.edge_target_df = pd.read_csv(Path(root) / edge_target_csv, index_col=['MoleculeName', 'BondID',])
            logger.info("DataFrame loaded successfully.")
            if self.edge_target_df.empty:
                logger.error("Target CSV file is empty.")
                raise EmptyDatasetError("Target CSV file is empty.")
        except FileNotFoundError:
            logger.error(f"Target CSV file not found: {edge_target_csv}")
            raise DatasetError(f"Target CSV file not found: {edge_target_csv}")
        ...
        self.data_processor.process_data(self.edge_target_df)  # Process data is now called here, and the edge_target_df is passed.
