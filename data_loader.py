...
class DataProcessor:
    ...
    def __init__(self, root: str, use_cache: bool = True, rdkit_config: Config = None, directed: bool = False):
        ...
        self.molecule_processor = MoleculeProcessor(root, rdkit_config=rdkit_config, directed=directed)
    ...
    def _process_molecule_graph(self, mol: Chem.Mol, mol_name: str, processed_name: str, edge_target_df: pd.DataFrame) -> None:
            ...
            if graph is not None:
                try:
                    ...
                    for bond in mol.GetBonds():
                        ...
                        bond_id = str(tuple(sorted((i, j))))
                        edge_target = edge_target_df.loc[(mol_name, bond_id,)].values
                        edge_targets.append(edge_target)
                        bond_index_list.append([i, j])
                        if not self.directed:
                            edge_targets.append(edge_target)
                            bond_index_list.append([j, i])
                    edge_targets = np.array(edge_targets)
                    bond_index_list = np.array(bond_index_list)
                    edge_targets = torch.tensor(edge_targets, dtype=torch.long).reshape(-1)
                    bond_index_list = torch.tensor(bond_index_list, dtype = torch.long).t().contiguous()
                    graph.edge_y = edge_targets
                    graph.edge_index = bond_index_list
    ...
    def process_data(self, edge_target_df: pd.DataFrame) -> None:
        ...
        for raw_path, processed_name in zip(raw_file_names, [Path(f).name.replace('.mol', '.pt') for f in raw_file_names]):
            ...
            self._process_molecule_graph(mol, mol_name, processed_name, edge_target_df)
