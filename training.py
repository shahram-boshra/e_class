...
def split_dataset(dataset: torch_geometric.data.Dataset, train_ratio: float, valid_ratio: float, edge_target_df: pd.DataFrame):
    """
    Splits a PyTorch Geometric dataset into train, validation, and test sets,
    with stratification based on edge_target values.
    """

    indices = list(edge_target_df.index)
    target_values = [edge_target_df.loc[idx].values[0] for idx in indices]
...
class TrainingLoop:
    ...
    def train_epoch(self, train_loader):
        ...
        for batch in train_loader:
            ...
            edge_out, l1_reg = self.model(batch)
            edge_target = batch.edge_y
            edge_loss = self.criterion(edge_out, edge_target)
            edge_loss += l1_reg * self.l1_lambda
            edge_loss.backward()
            self.optimizer.step()
            total_loss += edge_loss.item() * batch.num_edges
            num_edges += batch.num_edges
        self.step_lr.step()
        avg_loss = total_loss / num_edges
    ...
    def validate_epoch(self, valid_loader):
        ...
        with torch.no_grad():
            for batch in valid_loader:
                batch = batch.to(self.device)
                edge_out, _ = self.model(batch)
                edge_target = batch.edge_y
                edge_loss = self.criterion(edge_out, edge_target)
                total_loss += edge_loss.item() * batch.num_edges
                num_edges += batch.num_edges
                all_targets.append(edge_target.cpu().numpy())
                all_predictions.append(torch.argmax(edge_out, dim=1).cpu().numpy())
                all_probs.append(torch.softmax(edge_out, dim=1).cpu().numpy())
        ...
        # Correctly format probabilities for multiclass AUC
        if len(np.unique(all_targets)) > 2:
            auc = roc_auc_score(all_targets, all_probs, multi_class='ovr')
        else:
            auc = roc_auc_score(all_targets, all_probs[:, 1]) # binary case
