...
def split_dataset(dataset: torch_geometric.data.Dataset, train_ratio: float, valid_ratio: float, edge_target_df: pd.DataFrame):
    ...
    indices = list(edge_target_df.index) # change indices to be the index of the dataframe.
    target_values = [edge_target_df.loc[idx].values[0] for idx in indices]  # Access using loc and the correct index.
